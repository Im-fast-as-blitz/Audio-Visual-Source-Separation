...
def extract_feats(model, device, file_path, modality="video"):
    """
    :rtype: FloatTensor
    """
    model.eval()
    preprocessing_func = get_preprocessing_pipelines(modality)['test']
    data = preprocessing_func(np.load(file_path)['data'])  # data: TxHxW
...
def main():
    ...
    # dset_loader = get_data_loaders(args)

    assert os.path.isfile(args.model_path), \
        f"Model path does not exist. Path input: {args.model_path}"
    model = load_model(args.model_path, model, allow_size_mismatch=args.allow_size_mismatch, device=device)

    # feature extraction
    if args.mouth_patch_path is None:
        raise ValueError("mouth_patch_path param is not mentioned. Use this code only for embed inference.")
    
    for root, _, files in os.walk(args.mouth_patch_path):
        for file in files:
            out_path = os.path.join(args.mouth_embedding_out_path, file)
            in_path = os.path.join(root, file)
            save2npz(out_path, data = extract_feats(model, device, in_path).cpu().detach().numpy())
